...
; (function ($, window, document, undefined) {
    ...
    ComboTree.prototype.init = function () {
        ...
        this._elemArrowBtn = $('<a href="javascript:void(0);" id="' + this.comboTreeId + 'ArrowBtn" class="comboTreeArrowBtn"><span class="comboTreeArrowBtnImg">▼</span></a>');
    ...
    ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
        debugger;
        var itemHtml = "",
            isThereSubs = sourceItem.subs.length > 0 ? true : false; //sourceItem.hasOwnProperty("subs");
    ...
    ComboTree.prototype.refreshInputVal = function () {
        ...
        if (this.options.isMultiple) {
            ...
            tempids = this._selectedItem.id;
        }
        debugger;
        if (this.options.hiddeninput) {
            console.log(tempids); //Dhaval
            $(this.options.hiddeninput).val(tempids)
            
           
            if (this.elemInputId == "justAnInputBoxFrom") {

                $("#Amount").val("");
                $("#progress").show();
                $.ajax({
                    url: "/AccountHead/GetLedgerAccount?HeadId=" + tempids + "",
                    type: "GET",
                    dataType: "json",
                    ...
                    success: function (res) {
                        $('#LedgerIdFrom')
                            ...
                            .remove()
                            .end()
                            .append('<option value="0">--Select Ledger--</option>')
                            .val('0');

                        debugger;
                        $.each(res, function (head, value) {
                            debugger;
                            $("#LedgerIdFrom").append($("<option></option>").val(value.acc_id).html(value.acc_name));
                        })
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        $.each(res, function (head, value) {
                            debugger;
                            $("#LedgerIdTo").append($("<option></option>").val(value.acc_id).html(value.acc_name));
                        })
